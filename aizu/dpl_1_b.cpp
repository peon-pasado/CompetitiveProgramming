...
int main() {
    while (cin >> n >> C) {
        ...
        vector<int> p(n), temp(n);
        iota(p.begin(), p.end(), 0);
        sort(p.begin(), p.end(), [&](int i, int j) {
            return 1ll*c[i]*w[j] > 1ll*c[j]*w[i];
        });
        transform(p.begin(), p.end(), temp.begin(), [&](int i) {return c[i];});
        c = temp;
        transform(p.begin(), p.end(), temp.begin(), [&](int i) {return w[i];});
        w = temp;
        mtm::MTMSolver MTM(c, w, {C});
        cout << MTM.solve()[0] << endl;
    }
    return 0;
}