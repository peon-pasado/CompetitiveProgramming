 ...
 * @tag slope_trick, dp
 * @idea
 *      - we can use dp(i, x) where
 *      i is the i-th block and x
 *      is the leftmost point of this
 *      block. dp(i, x) is minimum cost.
 *
 *      - we can see that:
 *
 *          dp(i, x) = dp(i-1, y) + |x - l|
 *              min_{y \in [x-(r[i-1]-l[i-1]), x+(r[i]-l[i])}
 *
 *      - dp(i) is a polyline with slopes =
 *      {-i, -(i-1), -(i-2), ..., -1, 0, 1, ..., (i-1), i}
 *
 *      - we can use slope trick to maintain this in O(n \log n)
 *
