 ...
 * @author Miguel Mini
 * @tag centroid decomposition, fft, number_theory
 * @idea
 *      - if we consider the paths that go through a
 *      node, we can find that amount if we find all
 *      the combinations of distances from that node
 *      and then remove the contribution from each
 *      subtree, clearly I need to use fft having a
 *      complexity equal to O (n logn)
 *
 *      - finally if we use centroid decomposition,
 *      we will have a complexity of O (n log^2 n)
 *
 */
