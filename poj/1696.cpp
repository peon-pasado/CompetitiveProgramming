...
int main() {
    ...
    while (t--) {
        int n;
        cin>>n;
        vector<pair<int, P> > plants;
        vector<bool> vis(n);
        for (int i=0; i<n; ++i) {
            int id;
            P p;
            cin>>id>>p;
            plants.push_back(make_pair(id, p));
        }
        int j = 0;
        for (int i=1; i<n; ++i) {
            if (plants[i].second.y < plants[j].second.y || (plants[i].second.y == plants[j].second.y && plants[i].second.x < plants[j].second.x)) {
                j = i;
            }
        }
        cout << n << " " << plants[j].first;
        vis[j] = 1;
        for (int i=0; i<n-1; ++i) {
            int new_j = -1;
            for (int k=0; k<n; ++k) {
                ...
                for (int l=0; l<n; ++l) {
                    if (vis[l]) continue;
                    if ((plants[k].second - plants[j].second).cross(plants[l].second - plants[j].second) >= 0) {
                        cnt += 1;
                    } else {
                        break;
                    }
                ...
                if (cnt == n-i-2) {
                    if (new_j == -1 || (plants[new_j].second - plants[j].second).norm2() > (plants[k].second - plants[j].second).norm2()) {
                        new_j = k;
                    }
                }
                vis[k] = 0;
