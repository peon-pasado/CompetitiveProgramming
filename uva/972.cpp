...
public:
  void solveOne(istream& in, ostream& out) {
    int n, m;
    while (in >> n) {
      vector<Segment<double>> p(n);
      double x = 0;
      rep(i, n) {
        double h, l;
        in >> h >> l;
        Point<double> a(x, h), b(x + l, h);
        p[i] = Segment<double>(a, b);
        x += l;
      }
      in >> m;
      vector<Segment<double>> q(m);
      x = 0;
      rep(i, m) {
        double h, l;
        in >> h >> l;
        Point<double> a(x, h), b(x + l, h);
        q[i] = Segment<double>(a, b);
        x += l;
      }
      double ans = 1e18;
      const double eps = 1e-8;
      for (int i = 0, j = 0; i < n && j < m;) {
        ans = min(ans, max(p[i].a.y, q[j].a.y));
        bool oi = 0, oj = 0;
        if (p[i].b().x < q[j].b().x + eps) oi = 1;
        if (q[j].b().x < p[i].b().x + eps) oj = 1;
        i += oi;
        j += oj;
      }
      out << ans << endl;
