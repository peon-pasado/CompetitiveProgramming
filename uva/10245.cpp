...
void upd_ans(const Point<double> &a, const Point<double> &b) {
...
bool cmp_y(Point<double> a, Point<double> b) {
    return a.y < b.y;
}

vector<Point<double>> t, a;
...
struct Solver {
    bool solve_one(istream &in, ostream &out) {
        ...
        mindist = 1e20;
        rec(0, n);
        if (mindist + 1e-8 >= 10000) cout << "INFINITY\n";
        else out << mindist << '\n';
        return 0;
    }
