...
struct Point
{
    ...
    T x, y;
    bool c;
    explicit Point(T x = 0, T y = 0, bool c=0) : x(x), y(y), c(c) {}
    ...
    P perp() const { return P(y, -x, c); }                             // rotates -90 degrees
...
struct Solver {
    ...
    bool solve_one(istream &in, ostream &out) {
        ...
        forn(i, n) in >> p[i];
        int ans = n*1ll*(n-1)*(n-2)/6;
        const double pi = acos(-1);
        auto calc = [&](vector<Point<int>>& p, Point<int> o) {
            vector<double> angles;
            forn(i, n) if (!(p[i] == o)) {
                double angle = (p[i]-o).angle();
                angles.emb(angle);
                angles.emb(angle + 2*pi);
            }
            sort(all(angles));
            int idx[2] = {0, 0};
            int ans = 0;
            forn(i, (n-1)) {
                while (idx[0] < sz(angles) && 2*angles[idx[0]] + 1e-8 <= pi + 2*angles[i]) idx[0]++;
                while (idx[1] < sz(angles) && angles[idx[1]] < pi + angles[i]) idx[1]++;
                ans += idx[1]-idx[0];
            }
            return ans;
        };
        forn(i, n) ans -= calc(p, p[i]);
        out << "There are " << ans <<" sites for making valid tracks\n";
        return 0;
    }
